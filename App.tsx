...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Potentially update html lang attribute here if needed for accessibility
  ...
  const translate = (key: TranslationKey, replacements?: Record<string, string | number>): string => {
    return t(language, key, replacements);
  };

  // Centralized error handler for Supabase operations, now using translate
  const handleSupabaseError = (e: any, contextKey: TranslationKey | string): string => {
    console.error(`Supabase error during ${contextKey}:`, e);
    const contextStr = typeof contextKey === 'string' ? contextKey : translate(contextKey);
    if (e && e.code === '42501') { // RLS violation
        return translate('dbAccessDeniedError', { code: e.code, table: e.table || 'target table' });
    }
    const message = e.message || 'Unknown error';
    return translate('operationFailedError', { context: contextStr, message });
  };
  ...
  }, [language, translate]); // Re-check if API key message needs translation on lang change
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (sessionsFromDB.length > 0) {
          ...
          const sortedByDate = [...sessionsWithMessages].sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          setActiveSessionId(sortedByDate[0]?.id || null);
        } else {
          const defaultSessionId = `session-${Date.now()}`;
          const defaultSessionName = `ውይይት 1`; // Default name, user will rename
          const defaultSession: ChatSession = {
            id: defaultSessionId, name: defaultSessionName, messages: [], createdAt: new Date(), mode: 'general', historyCleared: false,
  ...
  }, [apiKeyExists, currentUser, language, translate]); // Add language to re-fetch if error messages change lang
  ...
  const handleCustomLogin = async (email: string, password: string) => {
    ...
    try {
      ...
      if (user && user.password === password) { // Plaintext password check - INSECURE
      ...
      setAuthError(`${translate('loginFailedError')} ${e.message}`);
    } finally {
      setCustomAuthLoading(false);
    }
  };
  ...
  const handleCustomSignUp = async (name: string, email: string, password: string) => {
    ...
    try {
      ...
      const newUser = await dbService.addUser(name, email, password); // Plaintext password stored - INSECURE
      setCurrentUser(newUser);
      setAuthView('initial_choice');
    } catch (e: any) {
      setAuthError(`${translate('signUpFailedError')} ${e.message}`);
    } finally {
      setCustomAuthLoading(false);
    }
  };
  ...
  const handleNewSession = useCallback(async () => {
    ...
    const newSession: ChatSession = {
      id: newSessionId,
      name: `ውይይት ${chatSessions.length + 1}`, // Default name, user will rename
      messages: [],
      createdAt: new Date(),
      mode: 'general',
  ...
  }, [chatSessions.length, currentUser, language, translate]);
  ...
  const handleChangeMode = async (newMode: ChatMode) => {
    if (!activeSessionId || !currentUser || !currentUser.id) return;
    try {
      ...
      if (activeSessionId) deleteChatSessionHistory(activeSessionId); // Clears Gemini in-memory chat
    } catch (e:any) {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (selectedFile) {
      try {
        ...
        // For Gemini, we use the Part format directly.
        fileInfoForGemini = {
            inlineData: {
    ...
    if (activeSession.historyCleared) updatedSessionForUI.historyCleared = false;
    ...
    try {
      ...
      if (activeSession.historyCleared) {
        await dbService.updateChatSessionHistoryClearedStatus(activeSession.id, false, currentUser.id);
      }
      if (updatedSessionNameFromDb) {
       ...
       if(activeSession.historyCleared) {
           setChatSessions(prevSessions => prevSessions.map(s => s.id === activeSessionId ? {...s, historyCleared: true} : s));
       }
    ...
    try {
      ...
      const geminiHistory = prepareHistoryForGemini(messagesForHistory);
      
      // Construct the 'contents' for the current message to send to AI
      const currentMessageParts: any[] = [];
      if (userMessageText) {
          currentMessageParts.push({ text: userMessageText });
      }
      if (fileInfoForGemini) { // This is already in the Part format
          currentMessageParts.push(fileInfoForGemini);
      }
      ...
      const aiResponseText = await sendMessageToAI(
        activeSession.id, 
        currentMessageContent, // Send the full Content object for the current turn
        activeSession.mode, 
        geminiHistory 
      );
      ...
      if (errorMessage.includes("User location is not supported")) {
        displayError = translate('aiRegionError');
      } else if (errorMessage.includes("billing account")) {
        displayError = translate('aiBillingError');
      }
      ...
      try {
        await dbService.addMessage(activeSession.id, errorAiMessage, currentUser.id);
      } catch (eDb: any) {
        setError(handleSupabaseError(eDb, "saving AI error message"));
      }
    } finally {
      setIsLoading(false); textareaRef.current?.focus();
    }
  }, [userInput, selectedFile, selectedFilePreview, apiKeyExists, activeSession, isLoading, currentUser, language, setIsLoading, setError, setChatSessions, activeSessionId, setUserInput, setFileError, translate]);
  ...
  const handleDeleteSession = async () => {
    ...
    if (activeSessionId === sessionIdToDelete) {
        ...
        if (remainingSessions.length > 0) {
            ...
            setActiveSessionId(remainingSessions[newActiveIndex]?.id || remainingSessions[0]?.id);
        } else {
    ...
    try {
        ...
        deleteChatSessionHistory(sessionIdToDelete); // Clears Gemini in-memory chat
    } catch (e: any) {
        setError(handleSupabaseError(e, "deleting session"));
        setChatSessions(sessionsBeforeDelete); // Revert UI on error
  ...
  const handlePromptClearActiveSessionHistory = () => {
      if (!activeSessionId || !activeSession || activeSessionMessages.length === 0) return;
      setIsClearHistoryModalOpen(true);
  };

  const handleClearActiveSessionHistory = async () => {
    if (!activeSessionId || !activeSession || !currentUser || !currentUser.id) return;

    setIsLoading(true); setError(null);
    try {
        await dbService.clearMessagesForSession(activeSessionId, currentUser.id);
        await dbService.updateChatSessionHistoryClearedStatus(activeSessionId, true, currentUser.id);

        setChatSessions(prevSessions =>
            prevSessions.map(session =>
                session.id === activeSessionId ? { ...session, messages: [], historyCleared: true, history_cleared_by_user_id: currentUser.id } : session
            )
        );
        deleteChatSessionHistory(activeSessionId); // Clears Gemini in-memory chat
    } catch (e: any) {
        setError(handleSupabaseError(e, "clearing session history"));
    } finally {
        setIsLoading(false);
  ...
  const handleInstallClick = async () => {
      if (deferredInstallPrompt) {
        ...
        setDeferredInstallPrompt(null); // We've used the prompt, clear it
      } else {
  ...
  if (isLoading && chatSessions.length === 0 && apiKeyExists) {
     return (
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${theme === 'dark' ? 'bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-gray-100' : 'bg-gray-100 text-gray-800'}`}>
      ...
      <div className="flex flex-col flex-1 min-h-0"> {/* Changed h-screen to min-h-0 */}
        ...
        <div className={`flex items-center justify-end px-4 py-2 ${theme === 'dark' ? 'bg-slate-800/50' : 'bg-gray-50 border-b border-gray-200'} `}>
             {activeSession && activeSessionMessages.length > 0 && (
                <button onClick={handlePromptClearActiveSessionHistory}
                    className={`p-2 rounded-full text-xs sm:text-sm flex items-center space-x-1.5 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1
                        ${theme === 'dark' ? 'text-orange-400 hover:bg-slate-700 focus:ring-orange-500 focus:ring-offset-slate-800' : 'text-orange-600 hover:bg-gray-200 focus:ring-orange-500 focus:ring-offset-gray-50'}`}
                    aria-label={translate('clearHistoryTitle')} title={translate('clearHistoryTitle')}>
        ...
        <main ref={chatContainerRef} className={`flex-grow p-4 md:p-6 space-y-4 overflow-y-auto scroll-smooth ${theme === 'dark' ? '' : 'bg-zinc-200'}`} aria-live="polite">
          ...
          {activeSession && activeSessionMessages.length === 0 && !isLoading && !error && (
            <div className={`flex flex-col items-center justify-center h-full text-center ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
          ...
          {isLoading && activeSessionId && activeSessionMessages.length > 0 && (userInput.trim() || selectedFile) && (
        ...
        <footer className={`p-3 md:p-4 border-t ${theme === 'dark' ? 'bg-slate-800/40 border-slate-700/50' : 'bg-gray-50 border-gray-200/80'} backdrop-blur-sm`}>
                ...
                {CHAT_MODES.map(modeInfo => (
                  <button key={modeInfo.id} onClick={() => handleChangeMode(modeInfo.id)}
                    className={`px-3 py-1.5 rounded-full text-xs sm:text-sm flex items-center space-x-1.5 transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-1
                      ${activeSession.mode === modeInfo.id ? 'bg-purple-600 text-white font-semibold shadow-md'
                        : `${theme === 'dark' ? 'bg-slate-700 hover:bg-slate-600 text-gray-300 hover:text-white' : 'bg-gray-200 hover:bg-gray-300 text-gray-700 hover:text-gray-900'}`}
          ...
          <div className="max-w-3xl mx-auto flex items-end space-x-2 sm:space-x-3">
            ...
            <textarea ref={textareaRef} rows={1}
              ...
              disabled={isLoading || !apiKeyExists || !activeSessionId} aria-label="Chat input"/>
            <button onClick={handleSendMessage} disabled={isLoading || (!userInput.trim() && !selectedFile) || !apiKeyExists || !activeSessionId}
            className={`p-3 rounded-lg text-white transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-purple-500
      ...
      <ConfirmClearHistoryModal isOpen={isClearHistoryModalOpen} onClose={() => setIsClearHistoryModalOpen(false)} onConfirm={handleClearActiveSessionHistory}
        theme={theme} language={language} translate={translate}/>
    </div>
  );
};
export default App;